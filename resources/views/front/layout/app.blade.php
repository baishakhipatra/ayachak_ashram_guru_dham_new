...
<head>
    ...
    <link rel="shortcut icon" href="{{ $base_url }}frontend/images/favicon.png" type="image/x-icon">
        ...
        <div class="container">
            <div class="header_sec">
                <a href="{{asset('/')}}" class="logo_img">
                    <img src="{{$base_url.'/frontend/images/logo1.png'}}" alt="logo">
                </a>

                <form action="{{route('front.product.list')}}" class="search_from">
                    <input type="search" id="product-input" class="search_input" placeholder="Search for products" name="keyword">
                ...
                <div class="col-12">
                    <div class="footer_text">
                       ...
                       <ul class="footer_text_menu">
                            ...
                            <li><a href="{{route('front.customer.care')}}">Customer Care</a></li>
                        </ul>
                    </div>
                </div>
            </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{route('front.product.list')}}">
                        <input type="search" class="mobile_search" id="mobile_search" name="keyword" placeholder="Search for products">
 ...
 // script.js
    $(document).ready(function() {
        $('#product-input').on('keyup', function() {
            ...
            // Make an AJAX call to fetch products from the server
            $.ajax({
                url: 'https://www.win.luxcozi.com/OP/search/suggestion', // Replace with your server endpoint
                method: 'GET',
                data: { val: inputValue },
                success: function(response) {
                    const products = response.products; // Adjust according to your API response structure
                    if (products && products.length > 0) {
                    products.forEach(product => {
                        const itemDiv = $('<div class="autocomplete-item"></div>');
                        itemDiv.html(`<a href="${product.url}">
                        ...
                        itemDiv.on('click', function() {
                            $('#product-input').val(product.title);
                            autocompleteList.empty();
                        });
        ...
        $('#mobile_search').on('keyup', function() {
            const inputValue = $(this).val().toLowerCase();
            const mobileautocompleteList = $('#mobile-autocomplete-list');
            mobileautocompleteList.empty();
            if (!inputValue) {
                return false;
            }
                ...
                success: function(response) {
                    ...
                    if (products && products.length > 0) {
                        products.forEach(product => {
                            ...
                            itemDiv.on('click', function() {
                                $('#mobile_search').val(product.title);
                                mobileautocompleteList.empty();
                            });

                            mobileautocompleteList.append(itemDiv);
                        });
                    } else {
                        // Optionally handle the case where there are no products
        ...
        $(document).on('click', function(e) {
            if (!$(e.target).closest('#mobile_search').length) {
                $('#mobile-autocomplete-list').empty();
            }
        });
    });
